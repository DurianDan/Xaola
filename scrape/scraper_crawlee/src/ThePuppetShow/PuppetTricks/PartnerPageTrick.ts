...
export default class PartnerPageTrick<P, E> implements BaseTrick<P, E> {
  ...
  async extractAvgRating(): Promise<number> {
    const foundRatingLine = await (
      await this.puppetMaster.selectElement(this.elements.avgRatingElement)
    )?.text();
    if (!foundRatingLine) {
      return 0;
    }
  ...
  async extractPartnerName(): Promise<string | undefined> {
    const foundNameLine = await (
      await this.puppetMaster.selectElement(this.elements.nameLineElement)
    )?.text();
  ...
  async extractPartnerInfo(): Promise<ShopifyPartner> {
    ...
    return new ShopifyPartner(
      new Date(),
      partnerName,
      this.urls.appPartnerLandingPage.toString(),
      appCount,
      avgRating,
    );
  }
  async extractDerive(): Promise<RawScrapeResult> {
    ...
    const appsInfo = await this.extractAppsInfo(
      foundPartner.id as string | undefined,
    );
    return {
